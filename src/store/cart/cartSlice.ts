  ...
  reducers: {
    ...
    removeQty: (state, action: PayloadAction<ItemQty>) => {
      ...
      if (qty > 0) {
        ...
        (state.items = updated), (state.itemCount = newItemCount);
        state.total = newTotalAmount;
      }
    },
    removeItem: (state, action: PayloadAction<ItemQty>) => {
      ...
      let newTotalAmount = removed.reduce((val, index) => val + index.price, 0);
      state.items = removed;
      state.itemCount = newItemCount;
      state.total = newTotalAmount;
    },
...
export const { addToCart, addQty, removeQty, removeItem, clearCart } =
  cartSlice.actions;
